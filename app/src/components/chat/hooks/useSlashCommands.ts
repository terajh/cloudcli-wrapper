...
export interface SlashCommand {
  ...
  path?: string;
  type?: string;
  /** 'claude' | 'codex' — only set for skills */
  agent?: string;
  /** 'user' | 'project' | 'plugin' | 'system' — only set for skills */
  scope?: string;
  /** 'command' | 'skill' | 'built-in' */
  kind?: string;
...
export function useSlashCommands({
    ...
    const fetchCommands = async () => {
      ...
      try {
        ...
        const allCommands: SlashCommand[] = [
          ...
          ...((data.skills || []) as SlashCommand[]).map((skill) => ({
            ...skill,
            type: 'skill',
            kind: 'skill',
          })),
  ...
  const selectCommandFromKeyboard = useCallback(
    (command: SlashCommand) => {
      ...
      // Skill은 자연어 트리거로 변환 ("Use the <name> skill: ")
      const isSkill = command.kind === 'skill';
      const skillName = command.name.replace(/^\//, '');
      const insertedText = isSkill ? `Use the ${skillName} skill: ` : `${command.name} `;
      const newInput = `${textBeforeSlash}${insertedText}${textAfterQuery}`;

      setInput(newInput);
      resetCommandMenuState();

      // Skill은 백엔드 execute 호출 없이 텍스트만 삽입
      if (isSkill) {
        // 텍스트 삽입 후 cursor 위치 조정
        requestAnimationFrame(() => {
          const ta = textareaRef.current;
          if (ta) {
            const pos = textBeforeSlash.length + insertedText.length;
            ta.focus();
  ...
  const handleCommandSelect = useCallback(
    (command: SlashCommand | null, index: number, isHover: boolean) => {
      ...
      // Skill: 자연어 트리거를 textarea에 삽입하고 종료
      if (command.kind === 'skill') {
        const skillName = command.name.replace(/^\//, '');
        const insertedText = `Use the ${skillName} skill: `;
        const currentInput = input;
        // slash menu가 열려있다면 slash position 기준으로 교체, 아니면 끝에 append
        if (slashPosition >= 0) {
          const textBeforeSlash = currentInput.slice(0, slashPosition);
          const textAfterSlash = currentInput.slice(slashPosition);
          const spaceIndex = textAfterSlash.indexOf(' ');
          const textAfterQuery = spaceIndex !== -1 ? textAfterSlash.slice(spaceIndex) : '';
          const newInput = `${textBeforeSlash}${insertedText}${textAfterQuery}`;
          setInput(newInput);
          ...
          setInput(`${currentInput}${insertedText}`);
          requestAnimationFrame(() => {
            const ta = textareaRef.current;
            if (ta) {
              ta.focus();
