...
/**
 * Window in which the main-session-effect trusts pendingViewSessionRef to
 * identify the temp→real swap without triggering a refetch. Five seconds is
 * enough for the backend round-trip but small enough that a stale ref from a
 * previous submission can't accidentally bypass normal loading.
 */
const PENDING_VIEW_GRACE_MS = 5000;
...
function chatMessageToNormalized(
  ...
  // Preserve the incoming id if the caller provided one (e.g. the optimistic
  // `__placeholder_*` assistant message that must be found and removed later
  // when real streaming starts). Otherwise generate a fresh local id.
...
export function useChatSessionState({
  ...
  /**
   * D-1 (T2) — Split streaming + placeholder messages off of the main list.
   *
   * The final in-flight message (either the optimistic placeholder or the
   * live streaming delta) is peeled into `tailChatMessage` so the rest of
   * the list (`stableChatMessages`) keeps a stable referential identity
   * while tokens are flowing. This is what lets ChatMessagesPane avoid
   * reconciling 1000+ already-rendered MessageComponent siblings every
   * ~100ms during streaming — only the tail slot re-renders.
   */
  const { stableChatMessages, tailChatMessage } = useMemo(() => {
    if (chatMessages.length === 0) {
      return { stableChatMessages: chatMessages, tailChatMessage: null as ChatMessage | null };
    }
    const last = chatMessages[chatMessages.length - 1];
    const isPlaceholder = last?.isPlaceholder === true;
    const isStreaming =
      typeof (last as any)?.id === 'string' &&
      ((last as any).id as string).startsWith('__streaming_');
    if (isPlaceholder || isStreaming) {
      return {
        stableChatMessages: chatMessages.slice(0, -1),
        tailChatMessage: last,
      };
    }
    return { stableChatMessages: chatMessages, tailChatMessage: null as ChatMessage | null };
  }, [chatMessages]);
  ...
  useEffect(() => {
    ...
    // Temp → real session swap mid-stream: if any of these guards fire we
    // are inside the brief window between `session_created` and the first
    // projects_updated, and refetching from the server would wipe the
    // streaming messages that haven't been persisted yet. The chat would go
    // blank until the user manually clicked the session again.
    //
    // (a) `session_acquired` phase — the realtime handler just ran
    //     promoteSession() and flipped the phase ref. Pass through once and
    //     advance to 'live' so later refetches still work.
    // (b) currentSessionId already matches and the store has it (the
    //     original guard — covers cases where React dispatches the selection
    //     update before the phase ref flips).
    // (c) pendingViewSessionRef claims this session id within the grace
    //     window AND the store already has data for it — the submission
    //     bound to this view hasn't completed yet.
    // (d) the store already holds live streaming data for this session.
    //     This is the final RC1 stopgap: even if every other signal is
    //     confused, we must not wipe visible content.
    const phase = sessionLifecyclePhaseRef.current;
    const pendingView = pendingViewSessionRef.current;
    const storeHasSession = sessionStore.has(selectedSession.id);
    const slot = storeHasSession ? sessionStore.peekSlot(selectedSession.id) : undefined;

    const guardA = phase === 'session_acquired';
    const guardB = currentSessionId === selectedSession.id && storeHasSession;
    const guardC =
      pendingView?.sessionId === selectedSession.id &&
      Date.now() - pendingView.startedAt < PENDING_VIEW_GRACE_MS &&
      storeHasSession;
    const guardD =
      storeHasSession &&
      !!slot &&
      (slot.realtimeMessages.length > 0 || slot.status === 'streaming');

    if (guardA || guardB || guardC || guardD) {
      lastLoadedSessionKeyRef.current = sessionKey;
      // One-shot phase transition: once we've passed the guard for the
      // promoted session, move to 'live' so subsequent refetches aren't
  ...
  const visibleMessages = useMemo(() => {
    if (stableChatMessages.length <= visibleMessageCount) return stableChatMessages;
    return stableChatMessages.slice(-visibleMessageCount);
  }, [stableChatMessages, visibleMessageCount]);
