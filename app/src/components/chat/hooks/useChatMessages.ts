...
export function normalizedToChatMessages(messages: NormalizedMessage[]): ChatMessage[] {
  ...
  for (const msg of messages) {
    // Optimistic assistant placeholder: empty content is intentional and must
    // NOT be dropped by the empty-content guard below. Surface it as an
    // assistant ChatMessage with the isPlaceholder flag so MessageComponent
    // can render its shimmer branch.
    if (typeof msg.id === 'string' && msg.id.startsWith('__placeholder_')) {
      converted.push({
        id: msg.id,
        type: 'assistant',
        content: '',
