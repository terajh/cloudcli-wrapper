...
interface UseChatComposerStateArgs {
  ...
  sessionLifecyclePhaseRef: { current: SessionLifecyclePhase };
  scrollToBottom: () => void;
...
export function useChatComposerState({
  ...
  const executeCommand = useCallback(
    async (command: SlashCommand, rawInput?: string) => {
      ...
      // Skill은 백엔드 execute 엔드포인트가 없으므로
      // textarea에 자연어 트리거를 삽입하고 종료한다.
      if (command.kind === 'skill') {
        const skillName = command.name.replace(/^\//, '');
        const trigger = `Use the ${skillName} skill: `;
        setInput(trigger);
        inputValueRef.current = trigger;
        return;
      }
  ...
  const handleSubmit = useCallback(
    async (
      ...
      // Optimistic assistant placeholder: gives immediate visual feedback for
      // where the response will land, removing the jarring empty gap between
      // the user message and the first streaming token. The placeholder is
      // auto-filtered by useSessionStore.updateStreaming/appendRealtime on
      // first token, and cleaned up on `complete` if still present.
      const assistantPlaceholder: ChatMessage = {
        id: `__placeholder_${sessionToActivate}`,
        type: 'assistant',
        content: '',
        timestamp: new Date(),
        isPlaceholder: true,
      ...
      if (!effectiveSessionId && !selectedSession?.id) {
        ...
        // Mark this submission so the session-loading effect knows the
        // upcoming selectedSession change is the temp→real promotion and
        // not a fresh navigation. `tempId` lets promoteSession() find the
        // in-flight slot during `session_created`.
