...
export default function ChatComposer({
  ...
  // CommandMenu는 textarea 우측 상단 위쪽으로 띄움 (max-w-4xl 컨테이너 내부에서 자연스럽게 보이도록)
  const commandMenuPosition = textareaRect
    ? {
        top: Math.max(16, textareaRect.top - 316),
        // 메뉴를 textarea의 우측에 정렬 (메뉴 너비 ~400px 가정)
        left: Math.max(16, textareaRect.right - 400),
        bottom: window.innerHeight - textareaRect.top + 8,
      }
    : { top: 0, left: 16, bottom: 90 };
      ...
      {!hasQuestionPanel && <form onSubmit={onSubmit as (event: FormEvent<HTMLFormElement>) => void} className="relative mx-auto max-w-4xl">
        ...
        >
          ...
          <div ref={inputHighlightRef} aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden rounded-2xl">
            <div className="chat-input-placeholder block w-full whitespace-pre-wrap break-words pl-4 pr-12 pb-11 pt-3 text-sm leading-5 text-transparent">
          ...
          <div className="relative z-10">
            <textarea
              ...
              className="chat-input-placeholder block max-h-[40vh] min-h-[68px] w-full resize-none overflow-y-auto rounded-2xl bg-transparent pl-4 pr-12 pb-11 pt-3 text-sm leading-5 text-foreground placeholder-muted-foreground/50 transition-all duration-200 focus:outline-none sm:max-h-[300px]"
              style={{ height: '68px' }}
            />

            <div className="absolute hidden" style={{ display: 'none' }}>
