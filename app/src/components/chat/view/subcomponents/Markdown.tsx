...
/**
 * Defer Prism syntax highlighting until the browser is idle so the first
 * paint of a code block never blocks the main thread (Prism can take
 * 50-100ms for large blocks). The plain pre/code shown first uses the same
 * oneDark background color so the swap is visually subtle.
 */
const IDLE_HIGHLIGHT_TIMEOUT_MS = 500;
const ONE_DARK_BG = '#282c34';
const ONE_DARK_FG = '#abb2bf';
...
type IdleWindow = Window & {
  requestIdleCallback?: (cb: (deadline: IdleCallbackDeadline) => void, opts?: IdleRequestOptions) => IdleCallbackHandle;
...
function scheduleIdle(callback: () => void): () => void {
  ...
  if (typeof w.requestIdleCallback === 'function') {
    const handle = w.requestIdleCallback(() => callback(), { timeout: IDLE_HIGHLIGHT_TIMEOUT_MS });
    return () => {
  ...
  // Safari fallback: setTimeout 0 still yields to layout/paint.
...
const CodeBlock = ({ node, inline, className, children, ...props }: CodeBlockProps) => {
  ...
  // Defer Prism highlighting until the browser is idle. The plain block is
  // shown first to keep the first paint cheap; the highlighted block swaps in
  // once the main thread is free.
  useEffect(() => {
    if (shouldInline) return;
    const cancel = scheduleIdle(() => setIsHighlighted(true));
    return cancel;
  }, [shouldInline, raw]);
