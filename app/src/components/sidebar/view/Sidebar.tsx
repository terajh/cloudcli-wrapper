...
import SidebarProjectFilterModal, { getVisibleProjects } from './subcomponents/SidebarProjectFilterModal';
...
function Sidebar({
  ...
  // Vienna는 자체 fork를 관리하므로 업스트림 업데이트 배너를 비활성화
  ...
  const visibleProjectNames = useMemo(() => getVisibleProjects(), [hiddenVersion]);
  const projects = useMemo(
    () => rawProjects.filter((p) => visibleProjectNames.has(p.name)),
    [rawProjects, visibleProjectNames],
  );
  ...
  return (
    <>
      ...
      <SidebarProjectFilterModal
        projects={rawProjects}
        open={showProjectFilter}
        onClose={() => setShowProjectFilter(false)}
        onSave={() => setHiddenVersion((v) => v + 1)}
        t={t}
      />
    </>
  );
