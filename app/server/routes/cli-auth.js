...
async function checkClaudeCredentials() {
  ...
  // Priority 2: Check ~/.claude/.credentials.json for OAuth tokens (legacy)
  // Older Claude CLI versions stored OAuth tokens here.
  try {
    ...
    // file missing or unreadable → fall through to Keychain check
  }

  // Priority 3: macOS Keychain (Claude CLI 2.x)
  // Newer Claude Code CLI versions store OAuth credentials in macOS Keychain
        ...
        try {
          ...
          if (oauth && oauth.accessToken) {
            ...
            if (!isExpired) {
              return {
                authenticated: true,
                email: oauth.email || parsed.email || 'Authenticated',
                method: 'keychain'
              };
            }
          }
        } catch {
          // 토큰이 JSON이 아니더라도 비어있지 않으면 인증된 것으로 간주
          return {
            authenticated: true,
            email: 'Authenticated',
            method: 'keychain'
